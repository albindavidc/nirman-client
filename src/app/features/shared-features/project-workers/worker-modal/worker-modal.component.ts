...
export class WorkerModalComponent implements OnInit {
  ...
  private submitSearchExisting(): void {
    if (
      this.selectedProfessionals.length === 0 ||
      this.existingRoleControl.invalid
    )
      return;
  ...
  private submitCreateNew(): void {
    ...
    this.workerService
      .addWorker(workerData as Partial<Worker>)
      .pipe(
        switchMap((newWorker: Worker) =>
          this.projectService.addProjectWorkers(
            this.data.projectId,
            [newWorker.id],
            projectRole,
          ),
      ...
      .subscribe({
        next: () => {
          ...
          this.notificationService.success(
            'New worker created and added successfully',
          );
          this.dialogRef.close(true);
        },
        error: (error: any) => {
